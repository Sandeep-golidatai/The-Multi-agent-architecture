...
class MarketStandardsUseCaseAgent:
    ...
    def propose_use_cases(self, industry):
        ...
        return [
            f"Use Case 1 for {industry} - AI-driven automation.",
            f"Use Case 2 for {industry} - Predictive analytics for decision-making.",
            f"Use Case 3 for {industry} - Chatbot integration for customer service.",
        ]
...
if "company_output" in st.session_state:
    if st.button("Collect Kaggle Resources") and "resource_output" not in st.session_state:
        resource_assets = resource_asset_agent.collect_kaggle_resources(st.session_state["company_output"]["industry"])
        st.session_state["resource_output"] = resource_assets

    if "resource_output" in st.session_state:
        st.markdown(st.session_state["resource_output"])

# Final Proposal - Top Use Cases
st.subheader("4. Final Proposal")

if "market_output" in st.session_state:
    st.write("### Top Use Cases for the Customer:")
    for use_case in st.session_state["market_output"]["use_cases"]:
        st.write(f"- {use_case}")

    st.write("### References:")
    st.write(f"- [Industry trends for {st.session_state['company_output']['industry']}]({st.session_state['company_output']['industry']}_trends)")
    st.write(f"- [Kaggle Resources for {st.session_state['company_output']['industry']}]({st.session_state['company_output']['industry']}_resources)")
